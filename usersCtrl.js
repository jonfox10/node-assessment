...
module.exports = {
  getUser: (req, res, next) => {
    ...
    const { age, email, favorites} = req.query;
        if(age){
            res.status(200).send(users.filter(user => user.age < age));
        } else if (email){
          ...
          // in this case you are comparing a string to a string, so triple equals should be used.
            res.status(200).send(users.filter(user => user.email === email));
        } else if (favorites){
            res.status(200).send(users.filter(user => user.favorites.includes(favorites)));
        } else {
    ...
    getUserType: (req, res, next) => {
        ...
        //better way
        res.status(200).send(users.filter(user => user.type === uType));
    ...
    //Oh MAN!!!! THIS IS GREAT! THANKS BETH!
